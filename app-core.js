...
function applyModeState(st){var blank=emptyModeState(),src=st&&typeof st==='object'?st:{};MODE_DATA_KEYS.forEach(function(k){S[k]=JSON.parse(JSON.stringify(src[k]!==undefined?src[k]:blank[k]));});}
...
function save(){
  ...
  try{localStorage.setItem(KEY,json);storageOK=true;}
    ...
    /* 꽉 찼으면 옛 사본 자리를 비우고 한 번 더 */
    /* 이관에 성공했는지 확인되기 전에는 과거 백업을 지우지 않아요. */
    try{localStorage.setItem(KEY,json);storageOK=true;}catch(e2){storageOK=false;}
  }
...
function postMeetRowHTML(r,compact){
  var p=r.payload||{},who=requestFriendCode(sharedFriendOf(r));
  return '<section class="postmeet'+(compact?' compact':'')+'"><b>'+esc(who)+'님과 재밌었어?</b><small>'+esc([p.date?mdTxt(parseKey(p.date)):'',p.place||'장소 미정',p.what||'약속'].filter(Boolean).join(' · '))+'</small><div class="acts"><button class="b-save" data-act="postmeet-yes" data-id="'+esc(r.request_id)+'">웅 ㅎ.ㅎ</button><button class="b-ghost" data-act="postmeet-no" data-id="'+esc(r.request_id)+'">아니, 안 놀았어</button></div></section>';
}
...
function postMeetAnswer(rid,yes){
  ...
  var fid=sharedFriendOf(row),me=Sync.uid,lp=JSON.parse(JSON.stringify(row.payload||{}));(lp.feedback=lp.feedback||{})[me]=yes?'yes':'no';row.payload=lp;
  if(M.type==='notices')drawNotices();else softRender();
  sharedPatch(rid,function(p){var fb=p.feedback||{};fb[me]=yes?'yes':'no';p.feedback=fb;}).then(function(o){
    var fb=o.p.feedback||{};
    if(!yes)inAppToast('안 만난 약속으로 표시했어요');
    else if(fb[fid]!=='yes')inAppToast('웅! '+requestFriendCode(fid)+'님도 웅 하면 추억으로 자동 기록돼요');
    return friendRequestLoad();
  }).catch(function(){inAppToast('답을 저장하지 못했어요');});
}
...
function autoMemoryFromShared(){
  var sb=friendDb();if(!sb)return Promise.resolve();
  var jobs=[];if(!S.settings.memToasted)S.settings.memToasted={};
  (FriendSync.shared||[]).forEach(function(r){
    var p=r.payload||{},fb=p.feedback||{};
    if(fb[r.user_a]!=='yes'||fb[r.user_b]!=='yes')return;
    var has=(FriendSync.memories||[]).some(function(x){return x.id===r.request_id||x.request_id===r.request_id;});
    if(p.memoryCreated&&!has&&!FriendSync.memReload&&!(FriendSync.memTried||(FriendSync.memTried={}))[r.request_id]){FriendSync.memTried[r.request_id]=1;FriendSync.memReload=1;friendLoadMemories().then(function(){FriendSync.memReload=0;autoMemoryFromShared();softRender();});return;}
    if(p.memoryCreated||has){
      if(!S.settings.memToasted[r.request_id]&&has){S.settings.memToasted[r.request_id]=1;save();var mx=(FriendSync.memories||[]).find(function(x){return x.id===r.request_id;})||memoryRowFromShared(r),k=memoryKey(mx),n=k?friendPlaceCounts(sharedFriendOf(r))[k]||1:0;inAppToast('둘 다 웅! 추억으로 기록했어요'+(k?' · '+k+'에서 '+(n===1?'첫':n+'번째')+' 추억이에요!':''));}
      return;
    }
    var row=memoryRowFromShared(r),withId=Object.assign({id:r.request_id},row);
    /* 약속 id로 저장해서 두 사람이 동시에 기록해도 한 번만 남아요 */
    jobs.push(sb.from('planner_friend_memories').upsert(withId,{onConflict:'id',ignoreDuplicates:true}).then(function(x){
      if(x.error){if(Sync.uid!==r.user_a)return null;return sb.from('planner_friend_memories').insert(row).then(function(y){if(y.error)throw y.error;return true;});}
      return true;
    }).then(function(ok){if(!ok)return;var np=JSON.parse(JSON.stringify(p));np.memoryCreated=true;r.payload=np;return sharedPatch(r.request_id,function(q){q.memoryCreated=true;});}));
  });
  if(!jobs.length)return Promise.resolve();
  return Promise.all(jobs).then(function(){return friendLoadMemories();}).then(function(){autoMemoryFromShared();softRender();}).catch(function(){});
}
...
function setMemoryPhoto(file,memId){
  shrinkTo(file,900,0.72,function(url){
    ...
    sb.from('planner_friend_memories').update({photo:url}).eq('id',memId).then(function(r){if(r.error)throw r.error;row.photo=url;drawFriendMemories();inAppToast('사진을 넣었어요');}).catch(function(e){inAppToast(userMsg('사진을 저장하지 못했어요. 잠시 뒤 다시 해주세요',/photo|column/i.test(e&&e.message||'')?'사진 칸이 서버에 없어요. planner-friends.sql을 실행해주세요':(e&&e.message)));});
  });
}
...
function openFriendPlanner(id){
  ...
  var html='<div class="friend-planner-title"><div>'+friendAvatar(label,f.photo,'lg')+'<div><h3>'+esc(label)+'님 플래너</h3><small>'+esc(lastMetInfo(id))+'</small></div></div></div>'+
...
function drawFriendMemories(){
  ...
  var list=rows.length?rows.slice(0,30).map(function(x){var i=memoryInfo(x);return '<div class="memory-card">'+(i?placeIcon(i.kind):'')+'<div class="mc-body"><b>'+esc(memoryAreaLabel(x))+'</b><small>'+esc([x.visited_at?mdTxt(parseKey(x.visited_at)):'',x.place,x.what].filter(function(v){return !!v;}).join(' · ')||'기록만 남김')+'</small>'+(x.photo?'<img class="mem-photo" src="'+esc(x.photo)+'" alt="추억 사진" loading="lazy">':(!FriendSync.noMemPhoto?'<label class="mem-addphoto">+ 사진 넣기<input type="file" accept="image/*" data-memphoto="'+esc(x.id)+'" hidden></label>':''))+'</div></div>';}).join(''):'<div class="memory-empty">아직 함께 남긴 추억이 없어요. 첫 추억을 남겨봐요.</div>';
  openModal('<h3>'+esc(friendLabel(f))+'님과의 추억</h3>'+relationSummaryHTML(id)+friendMemoryBanners(id)+
    ...
    '<p class="hint" id="memory-msg">'+(FriendSync.memoryError?'서버에 저장이 안 되면 이 기기에 먼저 저장해요.':'')+'</p><div class="acts">'+(S.settings.showMeetMaker===false?'':'<button class="b-ghost" data-act="memory-appointment">이 장소로 약속 잡기</button>')+'<button class="b-save" data-act="save-friend-memory">추억 저장</button></div>'+
...
function friendSqlHint(e){var m=(e&&(e.message||e.details||e.code))||String(e||'');return userMsg('연결 중 오류가 났어요. 잠시 뒤 다시 해주세요.',(/does not exist|schema cache|Could not find|relation/i.test(m)?'Supabase에서 friend-invite-busy.sql을 한 번 실행해주세요.':/permission|row-level|42501/i.test(m)?'권한 설정이 막혀 있어요. friend-invite-busy.sql을 다시 실행해주세요.':'연결 중 오류가 났어요.')+' (자세히: '+m+')');}
function myBusyOn(k){
...
function meetHeadHTML(f){return '<div class="meet-head">'+friendAvatar(friendLabel(f),f.photo)+'<div><b class="meet-days-big">'+esc(friendLabel(f))+'</b><span class="meet-days">'+esc(lastMetInfo(f.id))+'</span></div></div>';}
...
function openChoice(r,remoteAt){
  ...
  var warnR=rn<ln?'<p class="hint warn2">⚠️ 계정 쪽 항목이 '+(ln-rn)+'개 더 적어요. 이걸 고르면 이 기기 내용이 사라져요.</p>':'';
  var warnL=ln<rn?'<p class="hint warn2">⚠️ 이 기기 항목이 '+(rn-ln)+'개 더 적어요. 이걸 고르면 계정 내용이 사라져요.</p>':'';
...
function letterCard(k){
  ...
  if(got&&got.text&&k<=tk){
    html+='<section class="card"><div class="card-h"><h3>'+(k===tk?'어제의 나에게 받은 말':'이날 받은 말')+'</h3></div>'+
...
function todoItem(t,o){
  ...
  return '<li class="todo '+(t.done?'done':'')+'" data-tid="'+t.id+'">'+
    ...
    '<span class="ttxt" data-act="edit-todo" data-id="'+t.id+'">'+esc(t.text)+(t.course&&!o.nocourse?'<em class="ctag" style="--c:'+courseColor(t.course)+'">'+esc(t.course)+'</em>':'')+ddHTML(t)+(t.time?'<em class="tag">'+timeShort(t.time)+'</em>':'')+(t.focusMin?'<em class="tag">집중 '+durLong(t.focusMin)+'</em>':'')+(t.carried&&!t.done?'<em class="tag">이월'+(Number(t.carryCount||0)>1?' '+Number(t.carryCount||0)+'회':'')+'</em>':'')+(t.splitParentId?'<em class="tag">분할</em>':'')+(o.tag?'<em class="tag">'+esc(tagOf(t))+'</em>':'')+'</span>'+
...
function drawWeeklyCanvas(canvas,model){
  var n=model.dates.length,W=n===7?1800:1500,H=1200,ctx=canvas.getContext('2d'),paper='#fffdf9',ink='#3b3530',sub='#9a9086',line='#e9e2d6',soft='#f1ebe0';
  ...
  ctx.fillStyle=ink;ctx.font='800 34px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText(first.getFullYear()+'년 '+(first.getMonth()+1)+'월 '+Math.ceil(first.getDate()/7)+'주차',50,38);
  ctx.fillStyle=sub;ctx.font='500 18px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText((first.getMonth()+1)+'/'+first.getDate()+' – '+(last.getMonth()+1)+'/'+last.getDate()+' · 주간 플래너',50,82);
  ...
  model.dates.forEach(function(c,i){
    ...
    ctx.fillStyle=(i>=5?'#d0705f':ink);ctx.font='700 17px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText(DAYS[dow(d)],x+18,top+17);
    ctx.fillStyle=ink;ctx.font='800 25px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText(String(d.getDate()),x+18,top+43);
    ...
    function row(text,color,meta){
      ...
      ctx.fillStyle=ink;ctx.font='600 13px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';var lines=canvasLines(ctx,text,colW-70,1);ctx.fillText(lines[0],x+25,y+8);
      if(meta){ctx.fillStyle=sub;ctx.font='500 11px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText(meta,x+25,y+24);}
      y+=meta?45:40;
    }
    ...
    if(!c.labels.length&&!c.items.length&&!c.todos.length){ctx.fillStyle=sub;ctx.font='500 13px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText('기록 없음',x+18,top+105);}
  });
  var fy=H-footer+8;ctx.fillStyle=ink;ctx.font='700 16px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';ctx.fillText('이번 주 할 일',left,fy);
  ctx.fillStyle=sub;ctx.font='500 13px Pretendard, Apple SD Gothic Neo, Noto Sans KR, sans-serif';
...
function sideTodo(k,title){
  ...
  var openT=sortTodos(list.filter(function(t){return !t.done;})),doneT=sortTodos(list.filter(function(t){return t.done;}));
  ...
  var nOpen=openT.length+openR.length,nDone=doneT.length+doneR.length,tot=nOpen+nDone,id='day:'+k;
  return '<section class="card"><div class="card-h"><h3>'+esc(title)+'</h3><span class="cnt">'+nOpen+' 남음 · '+nDone+' 완료</span></div>'+
    (tot?'<div class="bar"><i style="width:'+Math.round(nDone/tot*100)+'%"></i></div>':'')+
    ...
    '<ul class="todos">'+((openR.map(function(r){return rtLi(r,false);}).join('')+openT.map(function(t){return todoItem(t,{focus:k===todayKey(),today:true});}).join(''))||'<li class="todo-empty-plain">'+(nDone?'다 끝냈어요':'아직 적은 일이 없어요')+'</li>')+'</ul>'+
    (nDone?'<div class="donelbl">완료됨</div><ul class="todos">'+doneR.map(function(r){return rtLi(r,true);}).join('')+doneT.map(function(t){return todoItem(t,{today:true});}).join('')+'</ul>':'')+
    '</section>';
}
...
function drawTTScan(){
  ...
  box.innerHTML='<p class="hint"><b>'+TTScan.rows.length+'개 수업을 찾았어요.</b> 틀린 부분은 수정하고 필요 없는 수업은 삭제해주세요.</p><div class="ttscan-list">'+TTScan.rows.map(function(x,i){
    ...
    return '<div class="ttscan-card'+(pr.length?' bad':'')+'" data-scan-i="'+i+'"><div class="ttscan-grid">'+
      ...
      (pr.length?'<div class="ttscan-warn">⚠ '+esc(pr.join(' · '))+'</div>':'')+
...
function viewSettings(){
  ...
  var diaryShortcut='<section class="card"><button class="setrow diary-library-shortcut" data-act="open-diary-library"><span>일기장<small>과거는 읽기 · 오늘은 쓰기 · 미래는 잠금</small></span><span class="chev">›</span></button><button class="diary-today-write" data-act="open-diary">오늘의 '+diaryMinutes()+'분 일기쓰기</button><div class="setrow"><span>오늘의 일기 시간<small>일기 버튼과 타이머에 바로 반영돼요</small></span><div class="seg" style="margin:0">'+[5,10,20,30].map(function(n){return '<button data-act="set-diary-minutes" data-v="'+n+'" class="'+(diaryMinutes()===n?'on':'')+'">'+n+'분</button>';}).join('')+'</div></div></section>';
  ...
  var main=modeCard+chatHTML+diaryShortcut+
    '<section class="card">'+
      ...
      navRow('settings-friends','친구','친구 추가 · 공개 범위 · 약속 설정')+
    '</section>'+
    '<section class="card">'+
...
function inAppToast(t){var el=document.getElementById('app-toast');if(!el){el=document.createElement('div');el.id='app-toast';el.setAttribute('role','status');document.body.appendChild(el);}el.textContent=t;el.classList.add('on');clearTimeout(inAppToast.t);inAppToast.t=setTimeout(function(){el.classList.remove('on');},3500);}
...
function drawNotices(){
  ...
  req.forEach(function(r){var p=r.payload||{},n=requestFriendCode(r.from_user),f=FriendSync.friends.find(function(y){return y.id===r.from_user;});todo+='<div class="ntc">'+friendAvatar(n,f&&f.photo)+'<div class="fc-name"><b>'+esc(n)+' · 약속 요청</b><small>'+esc([p.what,p.date?slotText(p.date,p.start?toMin(p.start):null,p.start?(p.end?toMin(p.end):toMin(p.start)+60):null):'시간 미정',p.place||'장소 미정'].filter(Boolean).join(' · '))+'</small></div><button class="tbtn b-save" data-act="appointment-open" data-id="'+r.id+'">확인</button></div>';});
  upcomingPlaceDecisions().forEach(function(r){var p=r.payload||{},fid=sharedFriendOf(r),n=requestFriendCode(fid),f=FriendSync.friends.find(function(y){return y.id===fid;});todo+='<div class="ntc">'+friendAvatar(n,f&&f.photo)+'<div class="fc-name"><b>'+esc(n)+'님과 장소 정하기</b><small>'+esc((p.date?mdTxt(parseKey(p.date))+' 약속 · ':'')+'후보 '+cleanCandidates(p.placeCandidates).length+'곳')+'</small></div><button class="tbtn b-save" data-act="place-decide-open" data-id="'+esc(r.request_id)+'">고르기</button></div>';});
...
function friendAvatar(t,photo,size){var c=String(t||'?').trim().charAt(0)||'?';return '<span class="fav'+(size?' fav-'+size:'')+'">'+(photo?'<img src="'+esc(photo)+'" alt="">':esc(c))+'</span>';}
...
function openFriendSharePreview(id){
  ...
  var html='<h3>'+esc(friendLabel(f))+'님에게 보이는 내 화면</h3><p class="hint">공개할 항목을 눌러 켜거나 끌 수 있어요. 모두 끄면 프로필만 보여요.</p>'+
    '<div class="fc-share" style="margin:0 0 10px"><span>공개 범위</span><button class="chipt'+(f.outTimetable?' on':'')+'" data-act="friend-toggle-share" data-id="'+esc(id)+'" data-mode="timetable">시간표</button><button class="chipt'+(f.outExams?' on':'')+'" data-act="friend-toggle-share" data-id="'+esc(id)+'" data-mode="exams">시험</button><button class="chipt'+(f.outCalendar?' on':'')+'" data-act="friend-toggle-share" data-id="'+esc(id)+'" data-mode="calendar">일정</button></div>'+
...
function friendHTML(){
  ...
  var reqIn=(FriendSync.requests||[]).filter(function(r){return r.to_user===Sync.uid&&r.status==='pending';}).map(function(r){var p=r.payload||{},n=requestFriendCode(r.from_user),rf=FriendSync.friends.find(function(y){return y.id===r.from_user;});return '<div class="fcard req"><div class="fc-top">'+friendAvatar(n,rf&&rf.photo)+'<div class="fc-name"><b>'+esc(n)+' · '+esc(p.what||p.title||'약속')+'</b><small>'+esc([p.date?slotText(p.date,p.start?toMin(p.start):null,p.start?(p.end?toMin(p.end):toMin(p.start)+60):null):'',p.place].filter(function(x){return !!x;}).join(' · ')||'약속 요청')+'</small></div><button class="tbtn b-save" data-act="appointment-open" data-id="'+r.id+'">확인</button></div></div>';}).join('');
  var rows=sortedFriends().map(function(f){var n=friendLabel(f),pinned=friendPinIndex(f.id)>=0;
    return '<button class="fcard friend-list-row" data-act="friend-detail" data-id="'+f.id+'"><div class="fc-top">'+friendAvatar(n,f.photo)+'<div class="fc-name"><b>'+esc(n)+'</b><small>'+esc(lastMetInfo(f.id))+' · 추억 '+friendMemoryRows(f.id).length+'개</small></div><span class="friend-list-tail">'+(pinned?'★ ':'')+'›</span></div></button>';}).join('');
  ...
  return '<section class="card"><div class="card-h"><h3>친구</h3><span class="cnt">'+(FriendSync.loaded?(nf?nf+'명':'연결됨'):'확인 중')+'</span></div>'+
    ...
    (S.settings.showMeetMaker===false?'':'<div class="fsec">링크 약속</div><div class="lk-entry"><button class="tbtn pri" data-act="lk-new">새 링크</button><button class="tbtn" data-act="lk-list">내 링크 '+LinkSync.links.length+'</button></div>')+
    ...
    (S.settings.showMeetMaker===false?'':'<div class="fsec">약속 설정</div>'+availRow)+
...
function globalSearchRows(query){
  ...
  (S.diaries||[]).forEach(function(d){add('diary','일기',d.id,(d.text||d.note||'일기').slice(0,40),globalSearchDate(dkey(new Date(d.finishedAt))),d.text||d.note||'','9991');});
...
function drawDiaryLibrary(){
  ...
  M.diaryIndex=ix;
  var shown=ix>=0?rows[ix]:null,date=shown?new Date(shown.finishedAt):parseKey(key),isToday=key===today,isFuture=key>today;
  ...
  if(shown){
    book='<div class="diary-book"><div class="diary-book-inner"><div class="diary-pretty-date"><b>'+esc(dateTitle)+'</b><small class="diary-meta-line"><span class="diary-word-meta">&lt;'+esc(wordMeta)+'&gt;</span><span class="diary-count-meta">/'+esc(countMeta)+'</span></small>'+diaryMoodSavedHTML(shown.mood)+'</div>'+
      (isToday?'<button class="diary-cta-btn diary-cta-under-word" data-act="diary-write-date" data-date="'+esc(key)+'">오늘의 '+diaryMinutes()+'분 일기쓰기</button>':'')+
...
function finishDiarySave(){
  ...
  S.diaries=S.diaries||[];
  var dk=p.date||todayKey(),now=new Date(),fd=parseKey(dk);fd.setHours(now.getHours(),now.getMinutes(),now.getSeconds(),now.getMilliseconds());
  S.diaries.push({id:uid(),text:p.text||'',note:p.note||'',word:word.slice(0,12),mood:nemoMoodValid(p.mood)?p.mood:'',elapsed:Number(p.elapsed||0),targetMinutes:Number(p.targetMinutes||diaryMinutes()),finishedAt:fd.getTime()});
...
function addTodo(scope,key,text,draftId,due,course){
  ...
  S.todos.push({id:uid(),text:q.text,done:false,star:false,scope:scope,key:key,due:due||null,course:course||'',time:time,created:Date.now(),order:Date.now()});
...
function sendFriendReport(id){
  ...
  }).catch(function(e){if(msg)msg.textContent=/does not exist|schema cache/i.test(e&&e.message||'')?'신고 기능 서버 설정이 아직 필요해요. release-safety.sql을 실행해주세요.':'신고를 보내지 못했어요. 잠시 뒤 다시 시도해주세요.';});
}
...
function deleteAccountError(e){
  ...
  if(/function.*does not exist|schema cache|planner_delete_my_account/i.test(raw))return userMsg('계정 삭제 서버 설정이 아직 필요해요.','account-deletion.sql을 실행해주세요');
...
function act(a,e){
  ...
  switch(name){
    ...
    case 'friend-meet':if(S.settings.showMeetMaker===false){inAppToast('설정 → 생활 기록에서 약속 잡기를 켤 수 있어요');break;}openFriendMeet(id);break;
    ...
    case 'lk-new':if(S.settings.showMeetMaker===false){inAppToast('설정 → 생활 기록에서 약속 잡기를 켤 수 있어요');break;}openLinkCreate();break;
    ...
    case 'memory-appointment':{if(S.settings.showMeetMaker===false){inAppToast('설정 → 생활 기록에서 약속 잡기를 켤 수 있어요');break;}var mmWhat=$('#f-mwhat')?$('#f-mwhat').value.trim():'',memInfo=ppInfo(M.pp),memFriend=M.friendId;closeModal();openFriendMeet(memFriend,{info:memInfo,what:mmWhat});break;}
    ...
    case 'onboard-next':{
      ...
      if(ok==='mode'){S.settings.plannerMode=M.plannerMode||'university';save();}
        ...
        if(txt){S.todos.push({id:uid(),text:txt,done:false,star:false,scope:'day',key:todayKey(),due:null,course:'',created:Date.now(),order:Date.now()});}

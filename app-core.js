...
function normalize(s){
  ...
  /* 일기는 새벽 5시 전까지 전날 공부일에 붙어요.
     직전 버전이 00:00~04:59 기록을 달력 날짜로 옮긴 경우(calendarDateFixed)만 안전하게 되돌려요. */
  o.diaries.forEach(function(x){
    if(!x||!x.finishedAt)return;
    var ft=new Date(Number(x.finishedAt));if(isNaN(ft))return;
    if(x.calendarDateFixed===true&&ft.getHours()<5){
      var sk=(typeof studyDayKey==='function'?studyDayKey(ft):dkey(new Date(ft.getFullYear(),ft.getMonth(),ft.getDate()-1)));
      x.date=sk;x.studyDate=sk;x.studyDay5Restored=true;x.calendarDateFixed=false;
    }
  });
...
function friendRequestLoad(){
  ...
  var prevRequestRenderSig='';try{prevRequestRenderSig=JSON.stringify([FriendSync.requests||[],FriendSync.shared||[]]);}catch(e){}
  ...
  ]).then(function(rs){rs.forEach(function(r){if(r&&r.error)throw r.error;});FriendSync.requests=(rs[0].data||[]).concat(rs[1].data||[]);FriendSync.shared=(rs[2].data||[]).concat(rs[3].data||[]);mergeSharedAppointments(FriendSync.shared);autoMemoryFromShared();friendNotifyScan();showNextAppointmentRequest();var nextRequestRenderSig='';try{nextRequestRenderSig=JSON.stringify([FriendSync.requests||[],FriendSync.shared||[]]);}catch(e){}if(nextRequestRenderSig!==prevRequestRenderSig)softRender();}).catch(function(e){if(!/relation|planner_appointment|planner_shared/i.test(e&&e.message||''))FriendSync.error=e&&e.message||'약속 요청을 불러오지 못했어요';});
}
...
function friendInviteLoad(){
  var sb=friendDb();if(!sb)return Promise.resolve();
  var prevInviteRenderSig='';try{prevInviteRenderSig=JSON.stringify([FriendSync.invitesIn||[],FriendSync.invitesOut||[]]);}catch(e){}
...
var MAPS=['memos','logs','letters','focus','fsess','retro','weeklyRetro','hourNotes','routineDone','modeStates','schedulePrepTemplates'];
...
function homeStrip(){
  ...
  try{
    var mb=window.PLANON_MEONBYEOL;
    if(mb&&mb.on&&mb.on()){
      var nm=mb.charName?mb.charName():(mb.charType&&mb.charType()==='dol'?'먼돌이':'먼별이');
      var ex=mb.curExpr?mb.curExpr():'normal';
      var cf=mb.charSVG?mb.charSVG(ex,{noGear:false}):'';
      var line=ex==='sleepy'?'밤이 늦었어요. 오늘 몫만 하고 같이 쉬어도 돼요':(dn>=tot&&tot?'오늘 할 일을 다 해냈어요':'오늘도 옆에서 같이 계획을 채워가요');
      nemo='<div class="nemo-home-head character-home-head"><span class="character-home-char">'+cf+'</span><span class="nemo-copy"><b>'+esc(nm+(ex==='sleepy'?'가 졸려해요':'와 함께'))+'</b><small>'+esc(line)+'</small></span></div>';
    }
  }catch(e){}
...
function coupleTimelineItems(x){
  if(!x||!x.date)return [];
  var m=coupleDisplayMode(x),items=[];
  if(m==='hundred'||m==='both')coupleHundredNumbers(x).forEach(function(n){items.push({name:n+'일',date:coupleMilestoneDate(x,n),kind:'hundred'});});
  coupleAnniversaryYears(x).forEach(function(y){items.push({name:y+'주년',date:coupleAnniversaryDate(x,y),kind:'year'});});
  if(m==='special'||m==='both')COUPLE_SPECIALS.forEach(function(sp){items.push({name:sp.name,date:nextSpecialDate(x,sp,todayKey()),kind:'special'});});
  var seen={};items=items.filter(function(it){var k=it.name+'|'+it.date;if(seen[k])return false;seen[k]=1;return true;});
  items.sort(function(a,b){return a.date<b.date?-1:a.date>b.date?1:(a.name<b.name?-1:1);});
  return items;
}
function coupleInlineTimelineHTML(x){
  if(ddCategory(x)!=='couple')return '';
  var mode=coupleDisplayMode(x),label=mode==='hundred'?'100일 단위':mode==='special'?'커플 기념일':'100일 + 커플 기념일';
  return '<div class="couple-inline-wrap"><div class="couple-inline-head"><b>'+esc(label)+'</b><small>주년은 항상 표시</small></div><div class="couple-inline-list">'+coupleTimelineItems(x).map(function(it){return '<button type="button" class="couple-inline-item '+it.kind+'" data-act="view-dd" data-id="'+esc(x.id)+'"><span>'+esc(it.name)+'</span><b>'+esc(fullDateTxt(it.date))+'</b></button>';}).join('')+'</div></div>';
}
...
function drawDD(){
  ...
  var d=$('#f-ddd').value,pv=$('#f-ddprev');if(d&&pv){var tmp={category:M.category,mode:M.mode,date:d,yearly:$('#f-ddy').checked,one:couple?true:$('#f-ddo').checked,coupleDisplay:M.coupleDisplay||'hundred'};pv.textContent=couple?('다음 기념일 · '+ddLabel(tmp)):('오늘 기준 '+ddCount(tmp));}
}
function saveDD(){
...
function topItemHTML(i){
  var act=i.type==='exam'?'edit-exam':'view-dd',icon='';if(i.type==='dd'){var dx=S.ddays.find(function(x){return 'dd:'+x.id===i.id;});icon=dx?'<i class="ddcat-row" style="--c:'+dx.color+'">'+ddCatIconHTML(dx)+'</i> ':'';}
...
function navHTML(){
  ...
  try{if(window.PLANON_MEONBYEOL&&window.PLANON_MEONBYEOL.on&&window.PLANON_MEONBYEOL.on())tabs.splice(3,0,['meonbyeol',window.PLANON_MEONBYEOL.charType&&window.PLANON_MEONBYEOL.charType()==='dol'?'먼돌':'먼별']);}catch(e){}
  var navIcons={
    ...
    meonbyeol:'<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M6 18c0-4 3-7 7-7 1-4 8-5 10-1 4 0 6 3 5 7 1 4-2 8-7 8H12c-4 0-7-3-6-7z" fill="#FFFDF8" stroke="#B8A0DC" stroke-width="2"/><path d="M11 18h3M18 18h3" stroke="#5E5468" stroke-width="1.8" stroke-linecap="round"/><path d="M25 6l1 2.4 2.4 1-2.4 1-1 2.4-1-2.4-2.4-1 2.4-1z" fill="#F2C94C"/></svg>'
  };
...
function gridHTML(dates,o){
  ...
  var specials='';
  /* 주간 그리드는 수업/시간 블록에 집중하고 D-day·종일 일정은 날짜를 눌렀을 때 보여줘요. 일간 화면은 그대로 표시합니다. */
  if(o.day){
...
function reviewHTML(mon){
  ...
  return '<details class="card weekly-review-card"><summary class="weekly-review-summary"><span><b>'+(dkey(mon)===dkey(thisMon)?'이번 주 돌아보기':'이 주 돌아보기')+'</b><small>완료율 '+pct+'% · 집중 '+durLong(focus)+'</small></span><span class="cnt">반복 포함 '+dn+'/'+tot+' · 펼치기</span></summary><div class="weekly-review-body">'+
    ...
    (left.length?'<ul class="todos">'+left.map(function(t){return todoItem(t,{tag:true});}).join('')+'</ul>'+
      ...
      :'<div class="empty">'+(tot?'이 주에 적은 일은 다 끝냈어요':'이번 주 기록이 아직 없어요')+'</div>')+'</div></details>';
}
...
function viewTable(){
  ...
  var ddl=S.ddays.slice().sort(function(a,b){var ac=ddCategory(a)==='couple',bc=ddCategory(b)==='couple';if(ac!==bc)return ac?-1:1;var an=ddNext(a),bn=ddNext(b);return an<bn?-1:an>bn?1:0;});
  var ddHTML='<section class="card dday-list-card"><div class="card-h"><h3>D-day</h3><button class="tbtn" data-act="add-dd">+ 추가</button></div>'+
    (ddl.length?ddl.map(function(x){var nd=parseKey(ddNext(x)),isCouple=ddCategory(x)==='couple';
      return '<div class="dday-list-entry"><div class="setrow clickable-row" data-act="view-dd" data-id="'+x.id+'" role="button" tabindex="0"><span style="display:flex;align-items:center;gap:8px"><i class="ddcat-row" style="--c:'+x.color+'">'+ddCatIconHTML(x)+'</i><span>'+esc(x.title)+'<small>'+(nd.getMonth()+1)+'/'+nd.getDate()+(isCouple?' · '+(coupleDisplayMode(x)==='hundred'?'100일 단위':coupleDisplayMode(x)==='special'?'커플 기념일':'둘 다'):x.mode==='since'?' · 시작일':'')+(x.yearly?' · 매년':'')+(x.pin?' · 위에 표시':'')+'</small></span></span><b class="ddn">'+ddLabel(x)+'</b><button class="tbtn" data-act="edit-dd" data-id="'+x.id+'">수정</button></div>'+(isCouple?coupleInlineTimelineHTML(x):'')+'</div>';}).join('')
  ...
  var adHTML='<section class="card"><div class="card-h"><h3>일정</h3><button class="tbtn" data-act="add-schedule">+ 추가</button></div>'+
    (adl.length?adl.map(function(a){var md=function(k){return k?Number(k.slice(5,7))+'/'+Number(k.slice(8)):'';};var when=a.days?daysText(a.days)+((a.from||a.to)?' · '+md(a.from)+'~'+md(a.to):''):a.end?md(a.date)+'~'+md(a.end):(function(x){return (x.getMonth()+1)+'/'+x.getDate()+' ('+DAYS[dow(x)]+')';})(parseKey(a.date));
      return '<div class="setrow clickable-row schedule-list-row" data-act="view-ad-detail" data-id="'+a.id+'" role="button" tabindex="0"><span style="display:flex;align-items:center;gap:8px"><i class="cdot" style="--c:'+a.color+';margin:0"></i><span>'+esc(a.title)+'<small>'+esc(when)+'</small></span></span><span class="chev">›</span><button class="tbtn" data-act="edit-ad" data-id="'+a.id+'">수정</button></div>';}).join('')
  ...
  return '<section class="card"><div class="card-h"><h3>'+esc(fixedTitle)+'</h3><div class="tt-actions"><button class="tbtn" data-act="wallpaper-phone">시간표 저장</button><button class="tbtn" data-act="ttscan-open">'+esc(scanLabel)+'</button><button class="tbtn" data-act="paste-table">붙여넣기</button></div></div>'+
...
function viewSettings(){
  ...
  var main='<section class="card settings-overview"><div class="card-h"><h3>설정</h3><span class="cnt">기능 한눈에</span></div><p class="hint">원하는 항목을 눌러 세부 설정을 열어요. 첫 화면에는 설정값을 펼쳐두지 않아요.</p><div class="settings-quick-restore"><button class="settings-shop-restore tbtn" data-market-open>Planon Shop</button><button class="tbtn" data-act="open-diary-library">일기장 가기</button><button class="tbtn" data-act="settings-recipes">레시피 노트</button><button class="tbtn'+(S.settings.showTodoTab===false?'':' on')+'" data-act="toggle-todo-tab">할 일 칸 '+(S.settings.showTodoTab===false?'꺼짐':'켜짐')+'</button><button class="tbtn'+(S.settings.showWeeklyReview===false?'':' on')+'" data-act="toggle-weekly-review">주간 회고 '+(S.settings.showWeeklyReview===false?'꺼짐':'켜짐')+'</button><button class="tbtn" data-act="backup">백업·복원</button></div><div class="setrow diary-minutes-quick"><span>일기 시간<small>쓰기 화면 타이머에 바로 반영돼요</small></span><div class="seg" style="margin:0">'+[5,10,20,30].map(function(n){return '<button data-act="set-diary-minutes" data-v="'+n+'" class="'+(diaryMinutes()===n?'on':'')+'">'+n+'분</button>';}).join('')+'</div></div>'+
      ...
      hubRow('settings-profile','life','프로필','이름 · 사진 · 친구 ID · 기본 출발지','친구에게 보이는 정보')+
...
function diaryDateKey(x){
  /* 저장된 날짜를 우선하고, 날짜가 없는 옛 기록만 '새벽 5시 전 = 전날' 규칙으로 복원해요. */
  ...
  return isNaN(d)?diaryCalendarTodayKey():(typeof studyDayKey==='function'?studyDayKey(d):dkey(d));
}
...
function openDiaryLibrary(dateKey){
  var rows=diaryLibraryRows(),wanted=dateKey||diaryCalendarTodayKey(),ix=-1;
...
function drawDiaryLibrary(){
  var rows=diaryLibraryRows(),key=M.diaryDate||diaryCalendarTodayKey(),today=diaryCalendarTodayKey(),ix=-1;
  ...
  if(shown){
    ...
    book='<div class="diary-book"><div class="diary-book-inner"><div class="diary-pretty-date"><b>'+esc(dateTitle)+'</b><small>지난 기록</small></div><div class="diary-empty-wrap"><div class="diary-empty-face" aria-hidden="true"><span class="diary-point"></span><i></i></div><div class="diary-empty-title">이날의 기록을 지금 남겨도 돼요</div><button class="diary-cta-btn" data-act="diary-write-date" data-date="'+esc(key)+'">이날 '+diaryMinutes()+'분 일기쓰기</button><div class="diary-empty-sub" style="margin-top:10px">날짜가 지나도 그날의 일기를 채울 수 있어요.</div></div></div></div>';
  }
  ...
  openModal('<div class="diary-library"><div class="diary-library-top"><button class="ibtn" data-act="close">‹</button><div><h3>일기장</h3><small>지난 날도 쓰고 · 오늘도 기록하고 · 미래는 기다려요</small></div><button class="tbtn" data-act="diary-cal-today">오늘</button></div>'+diaryCalHTML(key)+dayNav+book+nav+'</div>');
  $('#modal').classList.add('full');
}
...
function drawDiary(dateKey){
  var targetKey=dateKey||U.diaryDate||diaryCalendarTodayKey(),todayDiary=diaryCalendarTodayKey();if(targetKey>todayDiary){openDiaryLibrary(targetKey);return;}U.diaryDate=targetKey;M={type:'diary'};var r=U.diaryRun,mins=diaryMinutes(),isToday=targetKey===todayDiary,targetDate=parseKey(targetKey);
...
function finishDiary(){
  ...
  diaryPause();
  var dk=U.diaryDate||diaryCalendarTodayKey(),isToday=dk===diaryCalendarTodayKey();
...
function finishDiarySave(){
  ...
  S.diaries=S.diaries||[];
  var dk=p.date||diaryCalendarTodayKey(),finishedAt=Date.now();

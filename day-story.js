...
function avatarHTML(id,own,mood,color){var c=validNemoColor(color)||(own?ownNemoColor():'');return '<span class="story-avatar story-avatar-nemo"'+(c?' style="--planner-color:'+c+'"':'')+'>'+storyNemoHTML(validMood(mood)?mood:(own?liveOwnMood():'basic'),'')+'</span>';}
...
function snap(k){var st=B.dayCloseStats(k),cs=coreStats(k),showStudy=B.logItemOn('study'),showWake=B.logItemOn('wake'),sv=B.logVal(k,'study'),wv=B.logVal(k,'wake'),mood=(k===B.todayKey()?liveOwnMood():computedMood(st.total,st.done,Date.now()));return {id:'dayclose-'+k,key:k,date:k,finishedAt:Date.now(),closedAt:new Date().toISOString(),done:st.done,total:st.total,doneCount:st.done,totalCount:st.total,classes:st.classes,focus:st.focus,coreDone:cs.done,coreTotal:cs.total,studyMinutes:showStudy&&sv!=null?Number(sv):null,wakeTime:showWake&&wv?trimWake(wv):null,nemoMood:mood,nemoColor:ownNemoColor(),comment:'',visibility:'private',highlightItems:[],showDoneCount:true,cardVersion:4,storySync:'local',serverId:''};}
function fromServer(r){var c={id:'dayclose-'+r.date,key:r.date,date:r.date,finishedAt:new Date(r.closed_at).getTime(),closedAt:r.closed_at,done:Number(r.done_count||0),total:Number(r.total_count||0),doneCount:Number(r.done_count||0),totalCount:Number(r.total_count||0),coreDone:Number(r.core_done||0),coreTotal:Number(r.core_total||0),studyMinutes:r.study_minutes==null?null:Number(r.study_minutes),wakeTime:trimWake(r.wake_time),nemoMood:validMood(r.nemo_mood)?r.nemo_mood:'',nemoColor:validNemoColor(r.nemo_color),comment:r.comment||'',visibility:r.visibility||'private',highlightItems:normalizeHighlightItems(r.highlight_items),showDoneCount:r.show_done_count!==false,cardVersion:4,storySync:'synced',serverId:r.id,userId:r.user_id};if(!c.nemoMood)c.nemoMood=closingMood(c);return c;}
function cardHTML(c){var parts='',showCount=c.showDoneCount!==false,high=normalizeHighlightItems(c.highlightItems);if(showCount)parts+='<div class="story-card-stat"><b>'+Number(c.doneCount!=null?c.doneCount:c.done||0)+' / '+Number(c.totalCount!=null?c.totalCount:c.total||0)+'</b><small>완료한 할 일</small></div>';parts+='<div class="story-card-stat"><b>★ '+Number(c.coreDone||0)+' / '+Number(c.coreTotal||0)+'</b><small>오늘의 핵심</small></div>';if(c.studyMinutes!=null)parts+='<div class="story-card-stat"><b>'+Number(c.studyMinutes||0)+'분</b><small>공부 시간</small></div>';if(c.wakeTime)parts+='<div class="story-card-stat"><b>'+esc(trimWake(c.wakeTime))+'</b><small>기상 시각</small></div>';var highHTML=high.length?'<div class="story-card-highlights"><small>오늘 한 일</small>'+high.map(function(x){return '<div><i>✓</i><span><b>'+esc(x.text)+'</b>'+(x.course?'<em>'+esc(x.course)+'</em>':'')+'</span></div>';}).join('')+'</div>':'';var mine=!c.userId||(AC()&&AC().uid&&c.userId===AC().uid),owner='<div class="story-card-owner">'+avatarHTML(c.userId,mine,closingMood(c),c.nemoColor)+'<span>'+esc(mine?(String(S().settings.profileName||'').trim()||'나'):friendName(c.userId))+'</span></div>';return '<article class="story-card">'+owner+'<div class="story-card-date">'+esc(fmtDate(c.key||c.date))+'</div><div class="story-card-title">오늘의 요약</div><div class="story-card-stats">'+parts+'</div>'+highHTML+'<div class="story-card-comment'+(c.comment?'':' empty')+'">'+esc(c.comment||'오늘의 한 줄을 남기지 않았어요')+'</div><div class="story-card-foot">PLAN:ON</div></article>';}
...
function uploadClosing(c){var db=DB(),a=AC();if(!db||!a.uid){c.storySync='pending';B.save();syncBanner();return Promise.resolve(false);}var row=rowForUpload(c);function insertRow(x){return db.from('day_closings').insert(x).select('*').single();}function accept(r){if(r.error)throw r.error;c.serverId=r.data.id;c.storySync='synced';if(r.data&&validMood(r.data.nemo_mood))c.nemoMood=r.data.nemo_mood;if(r.data&&validNemoColor(r.data.nemo_color))c.nemoColor=r.data.nemo_color;B.save();syncBanner();return true;}return insertRow(row).then(accept).catch(function(e){var msg=String(e&&e.message||e||'');if(/nemo_color/i.test(msg)){var noColor=Object.assign({},row);delete noColor.nemo_color;return insertRow(noColor).then(accept);}if(/nemo_mood|highlight_items|show_done_count|schema cache|PGRST204|42703/i.test(msg)){var legacy=Object.assign({},row);delete legacy.nemo_mood;delete legacy.nemo_color;delete legacy.highlight_items;delete legacy.show_done_count;return insertRow(legacy).then(accept);}if(/duplicate|23505/i.test(msg)){return db.from('day_closings').select('id').eq('user_id',a.uid).eq('date',c.date||c.key).maybeSingle().then(function(r){if(r&&r.data){c.serverId=r.data.id;c.storySync='synced';B.save();syncBanner();return true;}c.storySync='pending';B.save();syncBanner();return false;});}c.storySync='pending';B.save();syncBanner();if(B.toast)B.toast('카드는 이 기기에 저장했어요');return false;});}
...
function loadStories(force){var db=DB(),a=AC(),mine=closeFor(B.todayKey());if(!mine||!db||!a.uid){ST.loading=false;ST.loaded=true;ST.stories=[];ST.error='';decorate();return Promise.resolve([]);}if(!force&&ST.loading)return Promise.resolve(ST.stories);ST.loading=true;decorate();var cut=new Date(Date.now()-86400000).toISOString();return db.from('day_closings').select('*').eq('visibility','friends').gte('closed_at',cut).neq('user_id',a.uid).order('closed_at',{ascending:false}).then(function(r){if(r.error)throw r.error;var friendIds=(FS().friends||[]).map(function(f){return f.id;}),seen={},rows=(r.data||[]).filter(function(x){if(friendIds.indexOf(x.user_id)<0||seen[x.user_id])return false;seen[x.user_id]=1;return true;});ST.stories=rows.map(fromServer);ST.loaded=true;ST.loading=false;ST.error='';ST.lastLoad=Date.now();var ids=ST.stories.map(function(x){return x.serverId;});if(!ids.length){ST.reactions=[];decorate();return ST.stories;}return db.from('story_reactions').select('closing_id,from_user_id,emoji,created_at').in('closing_id',ids).then(function(q){if(!q.error)ST.reactions=q.data||[];decorate();return ST.stories;});}).catch(function(){ST.loading=false;ST.loaded=true;ST.error='network';ST.stories=[];decorate();return [];});}
...
function refreshComposer(){var d=window.__planonCloseDraft;if(!d)return;var p=document.getElementById('story-compose-preview');if(p)p.innerHTML=cardHTML(d);var box=document.getElementById('story-task-picker-wrap');if(box)box.innerHTML=pickerHTML(d);var cb=document.getElementById('story-show-count');if(cb)cb.checked=d.showDoneCount!==false;}
...
function friendStoryHTML(c){return '<button data-story-act="view-friend-story" data-id="'+esc(c.userId)+'">'+avatarHTML(c.userId,false,closingMood(c),c.nemoColor)+'<span>'+esc(friendName(c.userId))+'</span></button>';}
...
document.addEventListener('change',function(e){if(e.target&&e.target.id==='story-show-count'&&window.__planonCloseDraft){window.__planonCloseDraft.showDoneCount=!!e.target.checked;refreshComposer();}});
